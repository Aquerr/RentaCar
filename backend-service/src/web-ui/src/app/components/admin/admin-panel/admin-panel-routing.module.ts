...
const ADMIN_PANEL_ROUTES: Routes = [
  {
    ...
    canActivate: [() => inject(authenticatedGuard).isAuthenticated() && inject(authenticatedGuard).hasUserAuthority(Auth.ADD_VEHICLE)],
    title: 'title.admin-add-vehicle'
  },
  {
    path: 'remove-vehicle',
    component: AdminRemoveVehicleComponent,
    canActivate: [() => inject(authenticatedGuard).isAuthenticated() && inject(authenticatedGuard).hasUserAuthority(Auth.REMOVE_VEHICLE)],
    title: 'title.admin-remove-vehicle'
  },
  {
    path: 'reservation-update-status',
    component: ReservationStatusUpdateComponent,
    canActivate: [() => inject(authenticatedGuard).isAuthenticated() && inject(authenticatedGuard).hasUserAuthority(Auth.GET_ALL_RESERVATIONS)],
